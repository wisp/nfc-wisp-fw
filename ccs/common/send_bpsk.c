...
unsigned int numberOfBitsTX;	/**< Number of bits to transmit. Transmit function uses this to know how much of the buffer to send. */
unsigned int timer;
...
void initialize_bpsk(void)
{
	...
	TB0CTL = TBSSEL_2;
	TB0CCTL2 =	OUTMOD_3; // sets at TBCCR1 / resets at TBCCR0 (SEE BELOW)
#ifdef _6_78Mhz
...
void send_bpsk(void)
{
	...
	//This method is used so that we can send the START bit immediately after the STOP bit at the end of the while loop.
	//This method does not affect the EOF since both the START bit and beginning of EOF are logic 0
	...
	while(bpsk_send_bit_count < numberOfBitsTX)
	{
		...
		//Derek Thrasher - Need to load next bits during previous wait period in order to meet timing constraints.
		...
		while(bpsk_ct < 0x08)
		{
			...
			if(bpsk_ct < 0x07) //don't itterate on last bit since we determin 1st bit before entering while loop (1+7 = 8 = 1 byte)
			{
				next_bit_value = next_bit();
				//bpsk_send_bit_count = bpsk_send_bit_count + 1;
	...
	timer += 4;//give it a couple cycles to settle.
	while(TA0R < timer);
	DISABLE_PWM()

	/*Derek Thrasher
	 * Timer B (PWM) is reinitialized at the end of each transmission for the following reasons
	 * Since we are using the Timer in Up/Down mode there is no way to know weather or not the counter
	 * is in the up or down state. So reinitializing allows us to set it back to upstate and keeps send_bpsk consistent
	 * For this same reason we are able to set TB0R = TB0CCR0-1 in order to ensure that we get an imediate rising edge at the start of send_bpsk()
	 */
	TB0CTL |= TBCLR;
	TB0CTL = TBSSEL_2;		//set clock source back to sm_clk
	#ifdef _6_78Mhz
	TB0CCR0 = 4; 			//upper bound compare
	TB0CCR1 = 0;			//lower bound compare
	TB0R = 3; 				//ensures that we will get a rising edge imediately
